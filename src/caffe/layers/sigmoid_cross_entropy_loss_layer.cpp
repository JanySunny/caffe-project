...
void SigmoidCrossEntropyLossLayer<Dtype>::Forward_cpu(
  ...
  sigmoid_layer_->Forward(sigmoid_bottom_vec_, sigmoid_top_vec_);//sigmoid_layer_ 用于生成sigmoid结果
  // Compute the loss (negative log likelihood)//计算负对数似然函数
  // Stable version of loss computation from input data
  const Dtype* input_data = bottom[0]->cpu_data();//bottom[0]是输入的预测的结果，bottom[1]是标签值。bottom的维度都是(N×C×H×W)
...
void SigmoidCrossEntropyLossLayer<Dtype>::Backward_cpu(
  ...
  if (propagate_down[0]) {
    ...
    caffe_sub(count, sigmoid_output_data, target, bottom_diff);//caffe_add、 caffe_sub、 caffe_mul、 caffe_div 函数分别实现element-wise的加减乘除
