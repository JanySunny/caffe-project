...
class SoftmaxWithLossLayer : public LossLayer<Dtype> {
 ...
 protected:
  ...
  Blob<Dtype> prob_;//prob_作为SoftmaxLayer的输出blob
  /// bottom vector holder used in call to the underlying SoftmaxLayer::Forward
  vector<Blob<Dtype>*> softmax_bottom_vec_;//指针指向bottom[0]，作为SoftmaxLayer的输入blob
  /// top vector holder used in call to the underlying SoftmaxLayer::Forward
  vector<Blob<Dtype>*> softmax_top_vec_;//指针指向prob_
  /// Whether to ignore instances with a certain label.
  bool has_ignore_label_;
  /// The label indicating that an instance should be ignored.
  int ignore_label_;//被忽略的标签值
  /// How to normalize the output loss.
  LossParameter_NormalizationMode normalization_;

  int softmax_axis_, outer_num_, inner_num_;//维数、输出样本数
};
